...
// reset the subringExtDegree of uniform and Xe sampler
func (enc *Encryptor) ResetSubringExtDegree(subringExtDegree int) (err error) {
	subringExtDegree = max(subringExtDegree, 1)
	enc.uniformSampler.SetSubringExtDegree(subringExtDegree)
	switch xe := enc.params.Xe().(type) {
	case ring.DiscreteGaussian:
		xe.ExtDegree = subringExtDegree
		enc.xeSampler, err = ring.NewSampler(enc.prng, enc.params.RingQ(), xe, false)
	default:
	}
	return
}
...
func newEncryptor(params Parameters) *Encryptor {
	...
	subringExtDegree := 1
	switch xe := params.xe.DistributionParameters.(type) {
	case ring.DiscreteGaussian:
		subringExtDegree = max(xe.ExtDegree, 1)
	default:
	}
	...
	return &Encryptor{
		...
		uniformSampler:   ringqp.NewUniformSampler(prng, *params.RingQP(), subringExtDegree),
		basisextender:    bc,
	}
}
