...
type ParametersLiteral struct {
	...
	SubringSecretExtDegree                      *int                        // Default: 1
...
const (
	...
	// DefaultSubringSecretExtDegree is the default extension degree of the full ring w.r.t. the subring secret
	DefaultSubringSecretExtDegree = 1
...
func (p ParametersLiteral) GetSubringSecretExtDegree() (SubringSecretExtDegree int, err error) {
	if v := p.EphemeralSecretWeight; v == nil {
		SubringSecretExtDegree = DefaultSubringSecretExtDegree
	} else {
		SubringSecretExtDegree = max(*v, 1)

		var logExtDegree = int(math.Log2(float64(SubringSecretExtDegree)))
		if 1<<logExtDegree != SubringSecretExtDegree || logExtDegree > p.GetLogN() || (logExtDegree > 0 && logExtDegree < 1<<12) {
			return SubringSecretExtDegree, fmt.Errorf("subring secret extension degree error")
		}
	}
	return
}
