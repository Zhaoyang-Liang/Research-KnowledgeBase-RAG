...
//
// NOTE: subring ternary/Gaussian distributions included
...
//
// NOTE: subring sampling requires P to be set
type Ternary struct {
	P         float64
	H         int
	ExtDegree int
...
func (d DiscreteGaussian) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		...
		ExtDegree    int     `json:",omitempty"`
	}{d.Type(), d.Sigma, d.Bound, d.ExtDegree})
}
...
func (d Ternary) MarshalJSON() ([]byte, error) {
	return json.Marshal(struct {
		...
		ExtDegree int     `json:",omitempty"`
	}{Type: d.Type(), P: d.P, H: d.H, ExtDegree: d.ExtDegree})
}
...
func ParametersFromMap(distDef map[string]interface{}) (DistributionParameters, error) {
	...
	case ternaryDistName:
		...
		extdegree, errExtDegree := getIntFromMap(distDef, "ExtDegree")
		if errExtDegree != nil {
			extdegree = 1
		}
		return Ternary{P: p, H: h, ExtDegree: extdegree}, nil
	case discreteGaussianName:
		...
		extdegree, errExtDegree := getIntFromMap(distDef, "ExtDegree")
		if errExtDegree != nil {
			extdegree = 1 // defaults to the original ring itself
		}
		return DiscreteGaussian{Sigma: sigma, Bound: bound, ExtDegree: extdegree}, nil
	default:
