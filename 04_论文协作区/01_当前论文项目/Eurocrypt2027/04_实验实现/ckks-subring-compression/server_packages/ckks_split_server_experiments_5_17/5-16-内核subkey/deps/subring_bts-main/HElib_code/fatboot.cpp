...
public:
    GTestFatboot(Parameters param,
                 ...
                 bool isThick,
                 bool forceRadix2,
                 std::vector<long> StC_partition,
                 std::vector<long> CtS_partition,
                 long subringExtDegree,
                 bool dummy) : 
                                ...
                                //  partition(partition),
                                 ...
                                 context((preContextSetup(),
                                          helib::ContextBuilder<helib::BGV>()
                                              ...
                                              .setThick(isThick)
                                              .forceRadix2(forceRadix2)
                                              .StC_partition(StC_partition)
                                              .CtS_partition(CtS_partition)
                                              .subringExtDeg(subringExtDegree)
            ...
            { // multiple tests with same key
                ...
                else
                {
                    ...
                    if (!dummy) { // no correctness check for dummy computation...
                        for (long i = 0, errcnt = 0; i < val1.size() && errcnt < 10; i++)
                        {
                            if (val1[i] != val2[i]){
                                errcnt++;
                                std::cout << i << " is bad\n";
...
int main(int argc, char *argv[])
{
    ...
    double btsScale_flag = 8;
    amap.arg("scale", btsScale_flag, "|I|<scale*stddev");
    ...
    long subringExtDegree_flag = 1;
    amap.arg("extdeg", subringExtDegree_flag, "subring extension degree, N/N'");
    ...
    Parameters po2_real_params[] = {
        ...
        */
        ...
        Parameters( // d = 1, m = 65536
                ...
                // main seclvl = 81.13, total bits <= 1350
                // p, r, c, bits, h
                // 1300 -> 1671.4, 1400 -> 1761.1, 1380 -> 1731.4
                65537, 1, 4, 1380, 0, //  bits = ... < 1747
                // nthreads,seed,useCache,c_m,
                    1, 0, 1, 100, 
                // force_bsgs,force_hoist,chen_han,debug,scale
                    0, 0, force_chen_han, 0, 0, 
...
/*E*/   Parameters( // d = 18, C = 141, good dim, m = 50731
    ...
    // scale = 14 for 128 bit
    //       = 10.3 for 64 bit
    //       = 8 for 32 bit
    //       = 6.3 for 16 bit
    double btsScale = btsScale_flag;
    ...
    if (helib::is2power(m)) {
        ...
        if (dst_params == po2_real_params){
            if (i_arg == 0)
                // partition = {0, 4, 8, 12, 16};
                // time = 6, 19, 11, 12
                partition = {0, 5, 8, 12, 16};
                // time = 11, 9, 11, 12
                /**
                 * time for linear1 = 8.153345, linear2 = 45.512524, extract = 61.076527, total = 115.861833
bits for linear1 = 100.259258, linear2 = 173.257488, extract = 451.253992, min cap = 18.000023, after cap = 714.064105, after prod = 1338.575584
cap / time = 5.142374
                 */
            if (i_arg == 1) {
            ...
            // XXX: uncomment only for benchmarking thick baselines!!
    ...
    std::vector<long> StC_partition = {0, 6, 11, 16}, CtS_partition;
    if (subringExtDegree_flag == 1) {
        CtS_partition = {0, 4, 7, 12, 16};
    } else {
        CtS_partition = {0, 2, 3, 4, 7, 12, 16};
    }

    GTestFatboot test(dst_params[i_arg], noPrint, dry, encapSkHwt, 
        btsC, t, btsScale, newBtsFlag, newKSFlag, s2cFirstFlag, isThick, 
        forceRadix2_arg, StC_partition, CtS_partition, subringExtDegree_flag, dummy_arg);
    // 4, 8, 12, 16 -> 18, 11, 12
    // 4, 7, 12, 16 -> 
    test.run();
    test.TearDown();
