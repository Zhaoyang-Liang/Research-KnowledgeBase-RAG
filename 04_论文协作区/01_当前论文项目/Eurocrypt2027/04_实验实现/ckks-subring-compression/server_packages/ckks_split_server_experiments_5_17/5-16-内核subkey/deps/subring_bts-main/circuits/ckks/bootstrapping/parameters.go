...
type Parameters struct {
	...
	// SubringSecretExtDegree: Extension degree of the full ring w.r.t. the subring key
	SubringSecretExtDegree int
...
func NewParametersFromLiteral(residualParameters ckks.Parameters, btpLit ParametersLiteral) (Parameters, error) {
	...
	var SubringSecretExtDegree int
	if SubringSecretExtDegree, err = btpLit.GetSubringSecretExtDegree(); err != nil {
		return Parameters{}, err
	}
	...
	return Parameters{
		...
		SubringSecretExtDegree:  SubringSecretExtDegree,
...
func (p Parameters) GaloisElements(params ckks.Parameters) (galEls []uint64) {
	...
	// keys[i] = true => gen^i in Z_2N^* is included in the Galois keys
