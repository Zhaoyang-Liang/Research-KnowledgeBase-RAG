...
type Evaluator struct {
	Parameters
	*ckks.Evaluator
	SubringEncapsulationEvaluator *rlwe.Evaluator // Key switching from the regular secret to the subring secret may require out-of-chain P
	DFTEvaluator                  *dft.Evaluator
	SubringStCEvaluator           *dft.Evaluator // A rlwe.Evaluator is included in dft.Evaluator, which can be used for KS with out-of-chain P
	SubringCtSEvaluator           *dft.Evaluator
...
func NewEvaluator(btpParams Parameters, evk *EvaluationKeys) (eval *Evaluator, err error) {
	...
	extraCtSLevel := 0
	if evk.SubringParamsModUp != nil {
		extraCtSLevel = 1
	}
	switch btpParams.CircuitOrder {
	case ModUpThenEncode:
		if btpParams.CoeffsToSlotsParameters.LevelQ-btpParams.CoeffsToSlotsParameters.Depth(true)+extraCtSLevel != btpParams.Mod1ParametersLiteral.LevelQ {
	...
	case DecodeThenModUp:
		if btpParams.BootstrappingParameters.MaxLevel()-btpParams.CoeffsToSlotsParameters.Depth(true)+extraCtSLevel != btpParams.Mod1ParametersLiteral.LevelQ {
			return nil, fmt.Errorf("starting level and depth of Mod1ParametersLiteral inconsistent starting level of CoeffsToSlotsParameters")
		}
	case Custom:
	default:
	...
	if evk.SubringParamsModUp != nil {
		eval.SubringEncapsulationEvaluator = rlwe.NewEvaluator(evk.SubringParamsModUp, evk)

		ckksParam := ckks.Parameters{Parameters: *evk.SubringParamsCtS}
		evkCtS := *evk // do not modify evk
		evkCtS.MemEvaluationKeySet = evkCtS.RotationKeysSubringCtS
		eval.SubringCtSEvaluator = dft.NewEvaluator(ckksParam, ckks.NewEvaluator(ckksParam, &evkCtS))
	}

	if evk.SubringParamsStc != nil {
		ckksParam := ckks.Parameters{Parameters: *evk.SubringParamsStc}
		evkStC := *evk // do not modify evk
		evkStC.MemEvaluationKeySet = evkStC.RotationKeysSubringStC
		eval.SubringStCEvaluator = dft.NewEvaluator(ckksParam,
			ckks.NewEvaluator(ckksParam, &evkStC))
	}
	// eval.SubringStCEvaluator is needed in eval.initialize
	if err = eval.initialize(btpParams); err != nil {
		return
	}
	if evk.SubringParamsStc != nil {
		// add rotation indices to StC Evaluator
		// XXX: only for non BSGS mode
		for _, mat := range eval.S2CDFTMatrix.Matrices[eval.SlotsToCoeffsParameters.Depth(false)-eval.SlotsToCoeffsParameters.LevelsInSubringSecret:] {
			if mat.N1 == 0 {
				keys := utils.GetSortedKeys(mat.Vec)
				eval.SubringStCEvaluator.InsertAutomorphismIndices(eval.SubringStCEvaluator.GetParameters().GaloisElements(keys))
			}
		}
	}
	if evk.SubringParamsCtS != nil {
		for _, mat := range eval.C2SDFTMatrix.Matrices[:eval.CoeffsToSlotsParameters.LevelsInSubringSecret] {
			if mat.N1 == 0 {
				keys := utils.GetSortedKeys(mat.Vec)
				eval.SubringCtSEvaluator.InsertAutomorphismIndices(eval.SubringCtSEvaluator.GetParameters().GaloisElements(keys))
			}
		}
	}
...
func (eval Evaluator) checkKeys(evk *EvaluationKeys) (err error) {
	...
	if evk.EvkBeforeModUp == nil && eval.EphemeralSecretWeight != 0 {
		return fmt.Errorf("rlwe.EvaluationKey key dense to sparse is nil")
	}

	if evk.EvkAfterModUp == nil && eval.EphemeralSecretWeight != 0 {
...
func (eval *Evaluator) initialize(btpParams Parameters) (err error) {
	...
	var subringSecretCtSEncoder *ckks.Encoder
	if eval.SubringCtSEvaluator != nil {
		subringSecretCtSEncoder = eval.SubringCtSEvaluator.Encoder
	}
	if eval.C2SDFTMatrix, err = dft.NewMatrixFromLiteral(params, eval.CoeffsToSlotsParameters, encoder, subringSecretCtSEncoder); err != nil {
		return
	}

	var subringSecretStCEncoder *ckks.Encoder
	if eval.SubringStCEvaluator != nil {
		subringSecretStCEncoder = eval.SubringStCEvaluator.Encoder
	}
	if eval.S2CDFTMatrix, err = dft.NewMatrixFromLiteral(params, eval.SlotsToCoeffsParameters, encoder, subringSecretStCEncoder); err != nil {
		return
	}
...
func (eval Evaluator) ModUp(ctIn *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext, err error) {
	...
	if eval.EvkBeforeModUp != nil {
		if eval.SubringEncapsulationEvaluator != nil {
			if err := eval.SubringEncapsulationEvaluator.ApplyEvaluationKey(ctIn, eval.EvkBeforeModUp, ctIn); err != nil {
				return nil, err
			}
		} else if err := eval.ApplyEvaluationKey(ctIn, eval.EvkBeforeModUp, ctIn); err != nil {
			return nil, err
		}
	}

	params := eval.BootstrappingParameters
	if eval.SubringCtSEvaluator != nil { // overraise the modulus by one prime
		params = *eval.SubringCtSEvaluator.GetParameters()
	}
	...
	// key switch from sparse secret to main secret
	if eval.EvkAfterModUp != nil && eval.SubringParamsModUp == nil {
		...
		if scale := (eval.Mod1Parameters.ScalingFactor().Float64() / eval.Mod1Parameters.MessageRatio()) / ctIn.Scale.Float64(); scale > 1 {
			...
			ctIn.Scale = ctIn.Scale.Mul(rlwe.NewScale(scale)) // FIXME: slight loss of precision
		}
		...
		ks.GadgetProductHoisted(levelQ, ks.BuffDecompQP, &eval.EvkAfterModUp.GadgetCiphertext, ctTmp)
...
func (eval Evaluator) CoeffsToSlots(ctIn *rlwe.Ciphertext) (ctReal, ctImag *rlwe.Ciphertext, err error) {
	if eval.SubringCtSEvaluator == nil {
		...
		curMatrices := eval.C2SDFTMatrix
		depth := curMatrices.Depth(true)
		curMatrices.Format = dft.Standard
		curMatrices.Matrices = curMatrices.Matrices[:curMatrices.LevelsInSubringSecret]
		// copied code from CoeffsToSlotsNew for manual LevelQ assignment
		subringCtSParams := eval.SubringCtSEvaluator.GetParameters()
		ctReal = ckks.NewCiphertext(*subringCtSParams, 1, subringCtSParams.MaxLevelQ())
		if curMatrices.LogSlots == subringCtSParams.LogMaxSlots() {
			ctImag = ckks.NewCiphertext(*subringCtSParams, 1, subringCtSParams.MaxLevelQ())
		}
		t_start := time.Now()
		err = eval.SubringCtSEvaluator.CoeffsToSlots(ctIn, curMatrices, ctReal, ctImag)
		duration := time.Since(t_start)
		fmt.Printf("Subring CtS time: %s\n", duration)
		if err != nil {
			panic(err)
		}

		eval.ApplyEvaluationKey(ctReal, eval.EvkAfterModUp, ctReal)

		curMatrices.Format = eval.C2SDFTMatrix.Format
		curMatrices.Matrices = curMatrices.Matrices[curMatrices.LevelsInSubringSecret:depth]
		err = eval.DFTEvaluator.CoeffsToSlots(ctReal, curMatrices, ctReal, ctImag)
		return
	}
}
...
func (eval Evaluator) SlotsToCoeffs(ctReal, ctImag *rlwe.Ciphertext) (ctOut *rlwe.Ciphertext, err error) {
	if eval.SubringStCEvaluator != nil {
		curMatrices := eval.S2CDFTMatrix
		depth := curMatrices.Depth(true)
		curMatrices.Matrices = curMatrices.Matrices[:depth-curMatrices.LevelsInSubringSecret]
		curMatrices.Levels = curMatrices.Levels[:depth-curMatrices.LevelsInSubringSecret]
		ctOut, err = eval.DFTEvaluator.SlotsToCoeffsNew(ctReal, ctImag, curMatrices)
		if err != nil {
			panic(err)
		}
		if err2 := eval.SubringStCEvaluator.ApplyEvaluationKey(ctOut, eval.EvkInStC, ctOut); err2 != nil {
			panic(err2)
		}
		curMatrices.Matrices = curMatrices.Matrices[depth-curMatrices.LevelsInSubringSecret : depth]
		curMatrices.Levels = curMatrices.Levels[depth-curMatrices.LevelsInSubringSecret : depth]
		if err3 := eval.SubringStCEvaluator.SlotsToCoeffs(ctOut, nil, curMatrices, ctOut); err3 != nil {
			panic(err3)
		}
		return
	} else {
