	...
	for i := 0; i < BaseRNSDecompositionVectorSize; i++ {
		...
		for j := 0; j < BaseTwoDecompositionVectorSize[i]; j++ {
			...
			if mask != 0 {
				...
				// ring.MaskVec(cxInvNTT.Coeffs[i], j*pw2, mask, cw) // XXX: why two masks here? Also consider signed base2 decomp?
			}
...
// hybrid decompose c1 and write NTT results into BuffDecompQP
func (eval Evaluator) DecomposeSinglePAndBitDecomp(levelQ, levelP int, c1 ring.Poly, isNTT bool, gadgetCt *GadgetCiphertext, BuffDecompQPPtr *[]VectorQP) {
	...
	var cxInvNTT, cxNTT ring.Poly
	if isNTT {
		cxInvNTT = eval.BuffInvNTT
		ringQ.INTT(c1, cxInvNTT)
		cxNTT = c1
	} else {
		cxInvNTT = c1
		cxNTT = eval.BuffInvNTT
	...
	BaseRNSDecompositionVectorSize := gadgetCt.BaseRNSDecompositionVectorSize()
	BaseTwoDecompositionVectorSize := gadgetCt.BaseTwoDecompositionVectorSize()
	...
	if len(*BuffDecompQPPtr) != BaseRNSDecompositionVectorSize {
		*BuffDecompQPPtr = make([]VectorQP, BaseRNSDecompositionVectorSize)
	}
	BuffDecompQP := *BuffDecompQPPtr
	// Re-encryption with CRT decomposition for the Qi
	for i := 0; i < BaseRNSDecompositionVectorSize; i++ {
		if len(BuffDecompQP[i]) != BaseTwoDecompositionVectorSize[i] {
			BuffDecompQP[i] = make(VectorQP, BaseTwoDecompositionVectorSize[i])
		}
		if mask == 0 {
			// NOTE: DecomposeAndSplit leave the current Qi group untouched for nbPi >= 2 (in most cases)
			// this is OK in DecomposeSingleNTT since they are overwritten by the NTT value of current Qi group
			// but it will lead to incorrect results for our params
			// so we use DecomposeSingleNTT instead
			BuffDecompQP[i][0] = ringQP.NewPoly()
			eval.DecomposeSingleNTT(levelQ, levelP, levelP+1, i, cxNTT, cxInvNTT, BuffDecompQP[i][0].Q, BuffDecompQP[i][0].P)
			continue
		}
		// mask != 0
		for j := 0; j < BaseTwoDecompositionVectorSize[i]; j++ {
			BuffDecompQP[i][j] = ringQP.NewPoly()
			ring.MaskVec(cxInvNTT.Coeffs[i], j*pw2, mask, cw)

			for u, s := range ringQ.SubRings[:levelQ+1] {
				s.NTT(cw, BuffDecompQP[i][j].Q.Coeffs[u])
			}

			if ringP != nil {
				for u, s := range ringP.SubRings[:levelP+1] {
					s.NTT(cw, BuffDecompQP[i][j].P.Coeffs[u])
				}
			}
		}
	}
...
// cx[i][j] = decomposed w.r.t Qi and base2 index j, in NTT form
func (eval Evaluator) GadgetProductSinglePAndBitDecompLazyHoisted(levelQ int, BuffDecompQP []VectorQP, gadgetCt *GadgetCiphertext, ctQP *Element[ringqp.Poly]) {
	...
	for i := 0; i < BaseRNSDecompositionVectorSize; i++ {
		...
		for j := 0; j < BaseTwoDecompositionVectorSize[i]; j++ {
			...
			if i == 0 && j == 0 {
				...
				for u, s := range ringQ.SubRings[:levelQ+1] {
					s.MulCoeffsMontgomeryLazy(el[i][j][0].Q.Coeffs[u], BuffDecompQP[i][j].Q.Coeffs[u], ctQP.Value[0].Q.Coeffs[u])
					s.MulCoeffsMontgomeryLazy(el[i][j][1].Q.Coeffs[u], BuffDecompQP[i][j].Q.Coeffs[u], ctQP.Value[1].Q.Coeffs[u])
				}

				if ringP != nil {
					for u, s := range ringP.SubRings[:levelP+1] {
						s.MulCoeffsMontgomeryLazy(el[i][j][0].P.Coeffs[u], BuffDecompQP[i][j].P.Coeffs[u], ctQP.Value[0].P.Coeffs[u])
						s.MulCoeffsMontgomeryLazy(el[i][j][1].P.Coeffs[u], BuffDecompQP[i][j].P.Coeffs[u], ctQP.Value[1].P.Coeffs[u])
					}
				}

			} else {
				for u, s := range ringQ.SubRings[:levelQ+1] {
					s.MulCoeffsMontgomeryLazyThenAddLazy(el[i][j][0].Q.Coeffs[u], BuffDecompQP[i][j].Q.Coeffs[u], ctQP.Value[0].Q.Coeffs[u])
					s.MulCoeffsMontgomeryLazyThenAddLazy(el[i][j][1].Q.Coeffs[u], BuffDecompQP[i][j].Q.Coeffs[u], ctQP.Value[1].Q.Coeffs[u])
				}

				if ringP != nil {
					for u, s := range ringP.SubRings[:levelP+1] {
						s.MulCoeffsMontgomeryLazyThenAddLazy(el[i][j][0].P.Coeffs[u], BuffDecompQP[i][j].P.Coeffs[u], ctQP.Value[0].P.Coeffs[u])
						s.MulCoeffsMontgomeryLazyThenAddLazy(el[i][j][1].P.Coeffs[u], BuffDecompQP[i][j].P.Coeffs[u], ctQP.Value[1].P.Coeffs[u])
					}
				}
			}
			...
			if reduce%PiOverF == PiOverF-1 && ringP != nil {
				ringP.Reduce(ctQP.Value[0].P, ctQP.Value[0].P)
				ringP.Reduce(ctQP.Value[1].P, ctQP.Value[1].P)
			}
...
func (eval Evaluator) DecomposeSingleNTT(levelQ, levelP, nbPi, BaseRNSDecompositionVectorSize int, c2NTT, c2InvNTT, c2QiQ, c2QiP ring.Poly) {
	...
	if ringP != nil { // ringP = nil for some parameters using subring secret
		ringP = ringP.AtLevel(levelP)
	}
