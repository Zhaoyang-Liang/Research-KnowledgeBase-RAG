...
type MatrixLiteral struct {
	...
	// for subring secret
	LevelsInSubringSecret int   // the number of levels left in StC before switching to subring secret
	SubringExtDegree      int   // Default: 1
	Base2Decomp           int   // log2(B) in base 2 decomp for KS to subring secret and automorphisms
	ManualDepthSplit      []int // use user-designated depth for each DFT matrix, instead of auto-computed ones
}
...
//
// For StC with subring secret, generate Galois keys for the last KSLevel DFT matrices using the subring secret
// and generate the keys for all preceding levels using the original secret
//
// For CtS with subring secret, do not generate any Galois keys (since the noise-to-modulus ratio is not small enough in KS)
//
// NOTE: this function is used only in keygen, not in online computation
func (d MatrixLiteral) GaloisElements(params ckks.Parameters) (galEls []uint64) {
	...
	// for StC
	subringStCFlag := false
	if d.SubringExtDegree > 1 {
		secretExtDegree := params.SubringSecretExtDegree()
		if d.Type == HomomorphicDecode {
			if d.SubringExtDegree == secretExtDegree { // StC subring
				indexCtS = indexCtS[len(indexCtS)-d.LevelsInSubringSecret:]
				subringStCFlag = true
			} else { // StC large ring
				indexCtS = indexCtS[:len(indexCtS)-d.LevelsInSubringSecret]
			}
		} else if d.SubringExtDegree == secretExtDegree { // CtS subring
			indexCtS = nil
		} else { // CtS large ring
			indexCtS = indexCtS[d.LevelsInSubringSecret:]
		}
	}
	...
	for i, pVec := range indexCtS {
		if !subringStCFlag {
			...
			// no BSGS needed, reduce the indices modulo N'
			// XXX: repack is not handled
			rotations = addMatrixRotToList(pVec, rotations, 1, min(params.N()/(2*d.SubringExtDegree), slots), d.Type == HomomorphicDecode && logSlots < logN-1 && i == 0 && imgRepack)
		}
	}
...
// pass a second encoder to encoders for the subring secret parameter set
//
// 'params' provides:
// 1. LogMaxDimensions, LogN, LevelsConsumedPerRescaling
// 2. Q primes for scale generation
// 3. ringQP for constant computation in ltcommon.NewTransformation
func NewMatrixFromLiteral(params ckks.Parameters, d MatrixLiteral, encoders ...*ckks.Encoder) (Matrix, error) {
	...
	// when there is an extra level in subring CtS, increase level by one for correct scale generation
	Q := params.Q()
	if d.Type == HomomorphicEncode && d.SubringExtDegree > 1 {
		Q = encoders[1].GetRLWEParameters().Q()
		level += 1
	}
	...
	for i := range d.Levels {
		...
		scale := rlwe.NewScale(Q[level])

		for j := 1; j < nbModuliPerRescale; j++ {
			scale = scale.Mul(rlwe.NewScale(Q[level-j]))
		}
		...
		for j := 0; j < d.Levels[i]; j++ {
			//  no need to pass subringExtDegree into ltparams, just use logBSGSratio
			...
			encoder := encoders[0]
			// need to pass subring information in ltparams
			var mat ltcommon.LinearTransformation
			if d.Type == HomomorphicDecode && d.SubringExtDegree > 1 && level <= d.LevelsInSubringSecret {
				encoder = encoders[1]
				ltparams.LogBabyStepGiantStepRatio = -1
				ltparams.LevelQ = encoder.GetParameters().MaxLevelQ()
				ltparams.LevelP = encoder.GetParameters().MaxLevelP()
				mat = ltcommon.NewTransformation(encoder.GetParameters(), ltparams)
			} else if d.Type == HomomorphicEncode && d.SubringExtDegree > 1 && i < d.LevelsInSubringSecret {
				// CtS here
				encoder = encoders[1]
				ltparams.LogBabyStepGiantStepRatio = -1
				ltparams.LevelQ = encoder.GetParameters().MaxLevelQ()
...
func (eval *Evaluator) SlotsToCoeffs(ctReal, ctImag *rlwe.Ciphertext, stcMatrices Matrix, opOut *rlwe.Ciphertext) (err error) {
	...
	if ctImag != nil {
		...
		if err = eval.dft(opOut, stcMatrices.Matrices, opOut, stcMatrices.Levels...); err != nil {
			return fmt.Errorf("cannot SlotsToCoeffs: %w", err)
		}
	} else {
		if err = eval.dft(ctReal, stcMatrices.Matrices, opOut, stcMatrices.Levels...); err != nil {
			return fmt.Errorf("cannot SlotsToCoeffs: %w", err)
		}
	}
...
// i-th rescale happends after evaluating rescaleAfter[i] matrices since the (i-1)-th rescale
func (eval *Evaluator) dft(ctIn *rlwe.Ciphertext, matrices []ltcommon.LinearTransformation, opOut *rlwe.Ciphertext, rescaleAfter ...int) (err error) {

	inputLogSlots := ctIn.LogDimensions

	// Sequentially multiplies w with the provided dft matrices.
	if err = eval.LTEvaluator.EvaluateSequential(ctIn, matrices, opOut, rescaleAfter...); err != nil {
		return
	}
...
func (d MatrixLiteral) computeBootstrappingDFTIndexMap(logN int) (rotationMap []map[int]bool) {
	...
	if len(d.ManualDepthSplit) > 0 {
		if len(d.ManualDepthSplit) != maxDepth {
			panic("length of manualDepthSplit does not match factorization depth")
		}
		sum := 0
		for _, v := range d.ManualDepthSplit {
			sum += v
		}
		if sum != level {
			panic("sum of manualDepthSplit does not match level")
		}
		merge = d.ManualDepthSplit
	} else {
...
func (d MatrixLiteral) GenMatrices(LogN int, prec uint) (plainVector []ltcommon.Diagonals[*bignum.Complex]) {
	...
	if len(d.ManualDepthSplit) > 0 {
		...
		if sum != fftLevel {
			panic("sum of manualDepthSplit does not match fftLevel")
		}
		merge = d.ManualDepthSplit
	} else {
		for i := 0; i < maxDepth; i++ {
