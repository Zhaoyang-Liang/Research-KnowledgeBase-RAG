...
func (u *UniformSampler) SetSubringExtDegree(subringExtDegree int) {
	if u.samplerQ != nil {
		u.samplerQ.SubringExtDegree = subringExtDegree
	}
	if u.samplerP != nil {
		u.samplerP.SubringExtDegree = subringExtDegree
	}
}

// NewUniformSampler instantiates a new UniformSampler from a given PRNG.
func NewUniformSampler(prng sampling.PRNG, r Ring, subringExtDegree ...int) (s UniformSampler) {
	if r.RingQ != nil {
	...
	if len(subringExtDegree) > 0 {
		s.SetSubringExtDegree(subringExtDegree[0])
	}
