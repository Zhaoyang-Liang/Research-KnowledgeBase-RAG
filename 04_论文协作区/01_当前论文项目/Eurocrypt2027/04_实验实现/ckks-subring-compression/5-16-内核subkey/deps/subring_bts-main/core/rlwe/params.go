...
// return the extension degree of subring secret
func (p Parameters) SubringSecretExtDegree() int {
	switch xs := p.xs.DistributionParameters.(type) {
	case ring.Ternary:
		return max(1, xs.ExtDegree)
	default:
		return 1
	}
}
...
func NewParametersFromLiteral(paramDef ParametersLiteral) (params Parameters, err error) {
	...
	if paramDef.LogQ != nil || paramDef.LogP != nil {
		existingPrimes := make([]uint64, len(paramDef.P)+len(paramDef.Q))
		copy(existingPrimes[copy(existingPrimes, paramDef.P):], paramDef.Q)
		switch paramDef.RingType {
		case ring.Standard:
			LogNthRoot := utils.Max(paramDef.LogN+1, paramDef.LogNthRoot)
			q, p, err = GenModuli(LogNthRoot, paramDef.LogQ, paramDef.LogP, existingPrimes) //2NthRoot
			if err != nil {
		...
		case ring.ConjugateInvariant:
			...
			q, p, err = GenModuli(LogNthRoot, paramDef.LogQ, paramDef.LogP, existingPrimes) //4NthRoot
			if err != nil {
...
// XeMut returns a pointer to rlwe.Parameters.xe
func (p *Parameters) XeMut() *Distribution {
	return &p.xe
}
...
// NOTE: avoid generating qi & pi that are already in the existingPrimes
func GenModuli(LogNthRoot int, logQ, logP []int, existingPrimes []uint64) (q, p []uint64, err error) {
	...
	existingPrimesMap := make(map[uint64]struct{}, len(existingPrimes))
	for _, existingPrime := range existingPrimes {
		existingPrimesMap[existingPrime] = struct{}{}
	}
	...
	for bitsize, value := range primesbitlen {
		...
		if bitsize == 61 {
			if primes[bitsize], err = g.NextDownstreamPrimesNew(value, existingPrimesMap); err != nil {
			...
			if primes[bitsize], err = g.NextAlternatingPrimesNew(value, existingPrimesMap); err != nil {
				// accept larger primes
				if primes[bitsize], err = g.NextPrimesNew(value, existingPrimesMap); err != nil {
