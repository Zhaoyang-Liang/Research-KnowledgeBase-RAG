...
// Low-output-level no-B_k CKKS bootstrapping comparison.
//
// This 5-16 experiment compares:
//
//   1. Direct low-budget top bootstrapping:
//        ScaleDown -> ModUp -> top C2S -> top EvalMod -> top S2C
//
//   2. Factored low-budget split bootstrapping:
//        Split
//        -> parallel leaf ScaleDown
//        -> parallel leaf ModUp
//        -> parallel leaf C2S
...
//        -> Merge
//
// Both paths should be run with the same low bootstrapping modulus budget.
// The goal is not to emulate a full-output-level top BTS, but to compare
// direct top lowQ refresh with split lowQ refresh under the same output budget.
//
// The program prints:
...
// Suggested lowQ run:
//
//   go run . -logN=16 -layers=1 -run=all -leafEngine=lattigo \
//     -q0Bits=34 -circuitLevels=1 -circuitPrimeBits=20 -numP=0 \
//     -defaultScale=25 -splitFirstLeafPreprocess=true -parallelLeaves=true \
//     -reps=1 -outputTSV=results/lowq.tsv
//
...
func makeN15LowOutputBootstrappingParams(logN int, xs ring.Ternary, outputLevels int) (ckks.Parameters, bootstrapping.Parameters) {
	if outputLevels < 1 || outputLevels > 3 {
		panic(fmt.Errorf("n15 low-output preset supports outputLevels in [1,3], got %d", outputLevels))
	}

	logQ := make([]int, 1+outputLevels)
	logQ[0] = 40
	for i := 1; i <= outputLevels; i++ {
		logQ[i] = 31
	}
...
func makeN15QP849LikeBootstrappingParams(logN int, xs ring.Ternary) (ckks.Parameters, bootstrapping.Parameters) {
	return makeN15LowOutputBootstrappingParams(logN, xs, 3)
}
...
func makeBootstrappingParamsForPreset(logN int, cfg ExpConfig, preset string) (ckks.Parameters, bootstrapping.Parameters) {
	switch preset {
	case "", "default":
		return makeBootstrappingParams(logN, cfg)
	case "n15lout1":
		return makeN15LowOutputBootstrappingParams(logN, cfg.Xs(), 1)
	case "n15lout2":
		return makeN15LowOutputBootstrappingParams(logN, cfg.Xs(), 2)
	case "n15lout3":
		return makeN15LowOutputBootstrappingParams(logN, cfg.Xs(), 3)
	case "n15qp849":
		return makeN15QP849LikeBootstrappingParams(logN, cfg.Xs())
	default:
		panic(fmt.Errorf("unknown lattigoPreset %q; valid options: default, n15lout1, n15lout2, n15lout3, n15qp849", preset))
	}
}
...
func makeDirectEvaluator(logN int, sk *rlwe.SecretKey, cfg ExpConfig, preset string) (ckks.Parameters, bootstrapping.Parameters, *bootstrapping.Evaluator) {
	_, btpParams := makeBootstrappingParamsForPreset(logN, cfg, preset)
...
func makeLeafEvaluatorPoolNoBK(topBootParams ckks.Parameters, leafLogN int, skLeaf *rlwe.SecretKey, workers int, cfg ExpConfig, leafQCountOverride int, preset string) (ckks.Parameters, bootstrapping.Parameters, []leafBootstrapper) {
	if workers <= 0 {
		panic("workers must be positive")
	}

	_, leafBtpParams := makeBootstrappingParamsForPreset(leafLogN, cfg, preset)
...
func printParamDryRun(logN, layers int, cfg, leafCfg ExpConfig, preset string) {
	_, btpTop := makeBootstrappingParamsForPreset(logN, cfg, preset)
	paramsTop := btpTop.BootstrappingParameters

	leafLogN := logN - layers
	leafParams := makeParamsFromExactBasis(paramsTop, leafLogN)
	_, leafBtp := makeBootstrappingParamsForPreset(leafLogN, leafCfg, preset)

	fmt.Println("=== Parameter dry run ===")
	fmt.Printf("lattigo preset: %s\n", preset)
	...
	fmt.Printf("leaf schedule reference: logN=%d qCount=%d pCount=%d logQ=%.1f logQP=%.1f\n",
		leafBtp.BootstrappingParameters.LogN(),
		leafBtp.BootstrappingParameters.QCount(),
		leafBtp.BootstrappingParameters.PCount(),
		leafBtp.BootstrappingParameters.LogQ(),
		leafBtp.BootstrappingParameters.LogQP())
...
func main() {
	...
	lattigoPreset := flag.String("lattigoPreset", "default",
		"Lattigo bootstrapping preset: default | n15lout1 | n15lout2 | n15lout3 | n15qp849")
	...
	outputTSV := flag.String("outputTSV", "",
		"append a one-line TSV summary to this path; empty disables TSV output")
	flag.Parse()
	...
	// In the 5-16 lowQ experiment, "all" intentionally means only the two
	// methods under comparison: direct top lowQ and split lowQ. The inherited
	// subKey25 path is left available for manual checks but is not part of this
	// experiment.
	runSubKey25 := *runMode == "subKey25"
	...
	if cfg.H <= 0 {
		fmt.Printf("[config] leafEngine=%s, lattigoPreset=%s, anyuProfile=%s, anyuSubringLogN=%d, Xs = uniform ternary P=2/3 (H≈%d for N=2^%d), q0Bits=%d, circuitPrimeBits=%d, defaultScale=%d, topCircuitLevels=%d, topNumP=%d, leafCircuitLevels=%d, leafNumP=%d, leafQCount=%d, splitFirstModUp=%v, splitFirstLeafPreprocess=%v, stageTrace=%v\n",
			*leafEngine, *lattigoPreset, anyuCfg.Profile, anyuCfg.SubringLogN, int(float64(int(1)<<uint(*logN))*2.0/3.0), *logN, cfg.Q0Bits, cfg.CircuitPrimeBits, cfg.DefaultScaleBits, cfg.CircuitLevels, cfg.NumP, leafCfg.CircuitLevels, leafCfg.NumP, *flagLeafQCount, *splitFirstModUp, *splitFirstLeafPreprocess, *stageTrace)
	} else {
		fmt.Printf("[config] leafEngine=%s, lattigoPreset=%s, anyuProfile=%s, anyuSubringLogN=%d, Xs = sparse H=%d, q0Bits=%d, circuitPrimeBits=%d, defaultScale=%d, topCircuitLevels=%d, topNumP=%d, leafCircuitLevels=%d, leafNumP=%d, leafQCount=%d, splitFirstModUp=%v, splitFirstLeafPreprocess=%v, stageTrace=%v\n",
			*leafEngine, *lattigoPreset, anyuCfg.Profile, anyuCfg.SubringLogN, cfg.H, cfg.Q0Bits, cfg.CircuitPrimeBits, cfg.DefaultScaleBits, cfg.CircuitLevels, cfg.NumP, leafCfg.CircuitLevels, leafCfg.NumP, *flagLeafQCount, *splitFirstModUp, *splitFirstLeafPreprocess, *stageTrace)
	}
	if *leafEngine == "anyu" {
	...
	if *dryRunParams {
		if *leafEngine == "anyu" {
			...
			printParamDryRun(*logN, *layers, cfg, leafCfg, *lattigoPreset)
		}
		return
	}
	...
	if *leafEngine == "anyu" {
		...
		_, btpTop = makeBootstrappingParamsForPreset(*logN, cfg, *lattigoPreset)
	}
	paramsTop := btpTop.BootstrappingParameters
	...
	if runOurs {
		if *leafEngine == "anyu" {
			...
			paramsLeaf, _, leafEvals = makeLeafEvaluatorPoolNoBK(paramsTop, leafLogN, skLeaf, leafWorkers, leafCfg, *flagLeafQCount, *lattigoPreset)
		}
	...
	if *outputTSV != "" {
		if err := appendSummaryTSV(*outputTSV, *logN, leafLogN, *layers, numLeaves, cfg, paramsTop, paramsLeaf,
			bRan, bAvgL1Err, bAvgModUp, bAvgCtS, bAvgEvalMod, bAvgStC, bAvgTotal,
			oursRan, avgL1Err, avgSplit, avgScaleDown, avgModUp, avgCtS, avgEvalMod, avgStC, avgMerge, avgTotal); err != nil {
			panic(fmt.Errorf("append TSV summary: %w", err))
		}
		fmt.Printf("Wrote TSV summary: %s\n", *outputTSV)
	}
}

func appendSummaryTSV(
	path string,
	logN, leafLogN, layers, leaves int,
	cfg ExpConfig,
	paramsTop ckks.Parameters,
	paramsLeaf ckks.Parameters,
	bRan bool,
	bPrec float64,
	bModUp, bCtS, bEvalMod, bStC, bTotal time.Duration,
	oursRan bool,
	oursPrec float64,
	oursSplit, oursScaleDown, oursModUp, oursCtS, oursEvalMod, oursStC, oursMerge, oursTotal time.Duration,
) error {
	...
	if needHeader {
		fmt.Fprintln(f, "time\tlogN\tleafLogN\tlayers\tleaves\tq0Bits\tcircuitLevels\tcircuitPrimeBits\tnumP\tdefaultScale\ttopLogQ\ttopLogQP\ttopQCount\ttopPCount\tleafLogQ\tleafLogQP\tleafQCount\tleafPCount\tbaselineRan\tbaselinePrecBits\tbaselineModUpSec\tbaselineCtSSec\tbaselineEvalModSec\tbaselineStCSec\tbaselineTotalSec\toursRan\toursPrecBits\toursSplitSec\toursScaleDownSec\toursModUpSec\toursCtSSec\toursEvalModSec\toursStCSec\toursMergeSec\toursTotalSec\tspeedup")
	}

	leafLogQ, leafLogQP := math.NaN(), math.NaN()
	leafQCount, leafPCount := 0, 0
	if oursRan {
		leafLogQ = paramsLeaf.LogQ()
		leafLogQP = paramsLeaf.LogQP()
		leafQCount = paramsLeaf.QCount()
		leafPCount = paramsLeaf.PCount()
	}
	speedup := math.NaN()
	if bRan && oursRan && oursTotal > 0 {
		speedup = float64(bTotal) / float64(oursTotal)
	}

	fmt.Fprintf(f, "%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%.6f\t%.6f\t%d\t%d\t%.6f\t%.6f\t%d\t%d\t%t\t%.6f\t%.9f\t%.9f\t%.9f\t%.9f\t%.9f\t%t\t%.6f\t%.9f\t%.9f\t%.9f\t%.9f\t%.9f\t%.9f\t%.9f\t%.9f\t%.6f\n",
		time.Now().Format(time.RFC3339),
		logN, leafLogN, layers, leaves,
		cfg.Q0Bits, cfg.CircuitLevels, cfg.CircuitPrimeBits, cfg.NumP, cfg.DefaultScaleBits,
		paramsTop.LogQ(), paramsTop.LogQP(), paramsTop.QCount(), paramsTop.PCount(),
		leafLogQ, leafLogQP, leafQCount, leafPCount,
		bRan, bPrec, bModUp.Seconds(), bCtS.Seconds(), bEvalMod.Seconds(), bStC.Seconds(), bTotal.Seconds(),
		oursRan, oursPrec, oursSplit.Seconds(), oursScaleDown.Seconds(), oursModUp.Seconds(), oursCtS.Seconds(), oursEvalMod.Seconds(), oursStC.Seconds(), oursMerge.Seconds(), oursTotal.Seconds(),
		speedup)
