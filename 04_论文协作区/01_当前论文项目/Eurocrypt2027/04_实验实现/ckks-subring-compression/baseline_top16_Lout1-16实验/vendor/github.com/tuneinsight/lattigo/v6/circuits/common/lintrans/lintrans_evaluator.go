...
func (eval Evaluator) EvaluateMany(ctIn *rlwe.Ciphertext, linearTransformations []LinearTransformation, opOut []*rlwe.Ciphertext) (err error) {
	...
	// this needs to be allocated lazily during KS,
	// since the base2 decomp info is contained in EVKs
	var BuffDecompQPBase2Ptr = &[]rlwe.VectorQP{}

	// MultiplyByDiagMatrixBase2 handles both subring rotations and base2 decomposition
	subringExtDegree := eval.GetRLWEParameters().SubringSecretExtDegree()
	if subringExtDegree <= 1 {
	...
	for i, lt := range linearTransformations {
		...
		if lt.N1 == 0 {
			...
			} else { // subring secret with hybrid decomposition
				if err = eval.MultiplyByDiagMatrixBase2(ctIn, lt, opOut[i], BuffDecompQPBase2Ptr); err != nil {
					return
				}
			}
		} else { // TODO: BSGS + StC?
...
func (eval Evaluator) EvaluateSequential(ctIn *rlwe.Ciphertext, linearTransformations []LinearTransformation, opOut *rlwe.Ciphertext, rescaleAfter ...int) (err error) {
	var rescaleCDF []int
	if rescaleAfter == nil {
		rescaleCDF = make([]int, len(linearTransformations))
		for i := range rescaleCDF {
			rescaleCDF[i] = i + 1
		}
	} else {
		rescaleCDF = make([]int, len(rescaleAfter))
		copy(rescaleCDF, rescaleAfter)
		// CDF
		for i := 1; i < len(rescaleCDF); i++ {
			rescaleCDF[i] += rescaleCDF[i-1]
		}
		if rescaleCDF[len(rescaleCDF)-1] != len(linearTransformations) {
			panic("sum of rescaleAfter does not match number of lintrans")
		}
	}
	rescaleIdx := 0
...
func (eval Evaluator) MultiplyByDiagMatrixBase2(ctIn *rlwe.Ciphertext, matrix LinearTransformation, opOut *rlwe.Ciphertext, BuffDecompQPBase2 ...*[]rlwe.VectorQP) (err error) {
	...
	*opOut.MetaData = *ctIn.MetaData
	...
	if ringP != nil {
		...
		ct0TimesP.CopyLvl(levelQ, ctInTmp0)
	}
	...
	subringSlots := min(slots, eval.GetRLWEParameters().N()/
		(2*eval.Evaluator.GetRLWEParameters().SubringSecretExtDegree()))

	// partition the rotation indices into quotient groups modulo N'/2
	// map[subring rotation length] -> diagonal indices
	rotIndices := make(map[int][]int)
	for _, k := range keys {
		subringK := k & (subringSlots - 1)
		rotIndices[subringK] = append(rotIndices[subringK], k&(slots-1))
	}

	ctr := 0
	for k, diagIndices := range rotIndices {
		if k == 0 {
			continue
		}
		galElSubring := params.GaloisElement(k)

		var evk *rlwe.GaloisKey
		var err error
		if evk, err = eval.CheckAndGetGaloisKey(galElSubring); err != nil {
			return fmt.Errorf("cannot MultiplyByDiagMatrix: Automorphism: CheckAndGetGaloisKey: %w", err)
		}

		if evk.LevelP() != levelP {
			return fmt.Errorf("LinearTransformation.LevelP = %d != GaloiKey[%d].LevelP() = %d: ensure that the levelP of the linear transformation is the same as the levelP of the GaloisKeys", levelP, galElSubring, evk.LevelP())
		}

		if ctr == 0 {
			eval.DecomposeSinglePAndBitDecomp(levelQ, levelP, ctInTmp1, ctIn.IsNTT, &evk.GadgetCiphertext, BuffDecompQPBase2[0])
		}
		// Encryption of as' mod sigma_k^{-1}(s')
		eval.GadgetProductSinglePAndBitDecompLazyHoisted(levelQ, *BuffDecompQPBase2[0], &evk.GadgetCiphertext, cQP)
	...
	if rotIndices[0] != nil { // Rotation by zero
		for _, diagIdx := range rotIndices[0] {
			index := eval.AutomorphismIndex(params.GaloisElement(diagIdx))
			ringQ.AutomorphismNTTWithIndex(ctInTmp0, index, tmp0QP.Q)
			ringQ.AutomorphismNTTWithIndex(ctInTmp1, index, tmp1QP.Q)

			pt := matrix.Vec[diagIdx].Q
			if ctr == 0 {
				// keyswitch(c1_Q) = (d0_QP, d1_QP)
				ringQ.MulCoeffsMontgomery(pt, tmp0QP.Q, c0OutQP.Q)
				ringQ.MulCoeffsMontgomery(pt, tmp1QP.Q, c1OutQP.Q)
			} else {
				// keyswitch(c1_Q) = (d0_QP, d1_QP)
				ringQ.MulCoeffsMontgomeryThenAdd(pt, tmp0QP.Q, c0OutQP.Q)
				ringQ.MulCoeffsMontgomeryThenAdd(pt, tmp1QP.Q, c1OutQP.Q)
			}
...
func (eval Evaluator) MultiplyByDiagMatrix(ctIn *rlwe.Ciphertext, matrix LinearTransformation, BuffDecompQP []ringqp.Poly, opOut *rlwe.Ciphertext, BuffDecompQPBase2 ...*[][]ringqp.Poly) (err error) {
