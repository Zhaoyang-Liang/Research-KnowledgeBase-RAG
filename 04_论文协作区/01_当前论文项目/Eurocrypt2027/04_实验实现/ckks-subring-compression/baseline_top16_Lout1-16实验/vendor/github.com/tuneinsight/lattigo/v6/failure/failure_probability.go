// from https://github.com/gong-cr/FHE-Security-Guidelines/tree/main
package failure
...
func ProbabilitySimple(Xs *ring.Ternary, K, logN, logSlots int, rejectionScale float64) (logfailure float64) {
	var boundH float64
	if Xs.P != 0 {
		N := math.Exp2(float64(logN))
		Eh := Xs.P * N
		stddevh := math.Sqrt(N * 2.0 / 9)
		boundH = Eh + stddevh*rejectionScale
	} else {
		return Probability(Xs, K, logN, logSlots)
	}
	stddevK := math.Sqrt((boundH + 1) / 12)
	return Log2ErfC(float64(K)/(stddevK*math.Sqrt(2)), 256) + float64(logSlots+1)
}

func FindSuitableKSimple(Xs *ring.Ternary, logN, logSlots int, logfailure, rejectionScale float64) (K int) {
	var boundH float64
	if Xs.P != 0 {
		N := math.Exp2(float64(logN))
		Eh := Xs.P * N
	...
	return int(getCorrectionFactor(logfailure-float64(logSlots)-1)*stddevK + 0.5)
}
