...
// NOTE:
//   - Value's length = BaseRNSDecompositionVectorSize
//   - Value[i]'s length = BaseTwoDecompositionVectorSize[i], the number of digits in base2 decomposition in this basis
//   - Value[i][j]: an RLWE sample modulo QP (a length 2 vector in ringQP)
