...
// NextDownstreamPrimesNew returns the next k primes of the form 2^{BitSize} - k * {NthRoot} + 1,
// which are not included in existingPrimes
func (n *NTTFriendlyPrimesGenerator) NextDownstreamPrimesNew(k int, existingPrimes map[uint64](struct{})) (primes []uint64, err error) {
	primes = make([]uint64, k)
	...
	for i := range primes {
		for {
			...
			if _, existing := existingPrimes[primes[i]]; !existing {
				break
			}
		}
	}
...
// NextAlternatingPrimesNew returns the next k primes of the form 2^{BitSize} +/- k * {NthRoot} + 1.
// which are not included in existingPrimes
func (n *NTTFriendlyPrimesGenerator) NextAlternatingPrimesNew(k int, existingPrimes map[uint64](struct{})) (primes []uint64, err error) {
	primes = make([]uint64, k)
...
// Next primes, prime lengths may be larger than the desired value
func (n *NTTFriendlyPrimesGenerator) NextPrimesNew(k int, existingPrimes map[uint64](struct{})) (primes []uint64, err error) {
	primes = make([]uint64, k)

	for i := range primes {
		for {
			if primes[i], err = n.nextPrimeNew(); err != nil {
				return
			}
			if _, existing := existingPrimes[primes[i]]; !existing {
				break
...
// Just the next prine
func (n *NTTFriendlyPrimesGenerator) nextPrimeNew() (uint64, error) {
