...
func NewTernarySampler(prng sampling.PRNG, baseRing *Ring, X Ternary, montgomery bool) (ts *TernarySampler, err error) {
	...
	ts.extdegree = max(X.ExtDegree, 1) // handle possibly default initialized ExtDegree
	switch {
	case X.P != 0 && X.H == 0:
		ts.invDensity = 1 - X.P
		ts.sample = ts.sampleProba // supports subring sampling
		if ts.invDensity != 0.5 {
			ts.computeMatrixTernary(ts.invDensity)
		}
	case X.P == 0 && X.H != 0:
		ts.hw = X.H
		ts.sample = ts.sampleSparse // does not support subring sampling
	default:
...
func (ts *TernarySampler) sampleProba(pol Poly, f func(a, b, c uint64) uint64) {
	...
	step := ts.extdegree
	// Failure probability is erfc(2/sqrt(2)) < 2^-4.4
	hwt_thres := uint64((1-ts.invDensity)*float64(N/ts.extdegree) +
		13.1*math.Sqrt(float64(N/ts.extdegree)*ts.invDensity*(1-ts.invDensity)))
	var hwt uint64
			...
			for i := 0; i < N; i++ {
				if i%step != 0 {
					for j, qi := range moduli {
						pol.Coeffs[j][i] = f(pol.Coeffs[j][i], lut[j][0], qi)
					}
					continue
				}
